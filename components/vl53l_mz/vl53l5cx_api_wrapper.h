...
#if defined(VL53L_MZ_RUN_XTALK_CALIBRATION) || defined(VL53L_MZ_SET_XTALK_CALIBRATION_DATA)
static uint8_t vl53l5cx_xtalk_buffer[VL53L5CX_XTALK_BUFFER_SIZE];
#endif
...
public:
  ...
  uint8_t set_resolution(const uint8_t resolution) override {
    if (resolution == VL53LMZ_INVALID_OPTION) return VL53L5CX_STATUS_ERROR;
    return vl53l5cx_set_resolution(&this->config_, resolution);
  }
  ...
  uint8_t set_ranging_mode(const uint8_t ranging_mode) override {
    if (ranging_mode == VL53LMZ_INVALID_OPTION) return VL53L5CX_STATUS_ERROR;
  ...
  uint8_t set_target_order(const uint8_t target_order) override {
    if (target_order == VL53LMZ_INVALID_OPTION) return VL53L5CX_STATUS_ERROR;
  ...
  uint8_t get_caldata_xtalk(uint8_t *p_xtalk_data) override { return vl53l5cx_get_caldata_xtalk(&this->config_, p_xtalk_data); }

  uint8_t set_caldata_xtalk(uint8_t *p_xtalk_data) override { return vl53l5cx_set_caldata_xtalk(&this->config_, p_xtalk_data); }

  uint8_t *get_xtalk_buffer() override { return vl53l5cx_xtalk_buffer; }

  uint16_t get_xtalk_buffer_size() {return VL53L5CX_XTALK_BUFFER_SIZE; }
#endif
  ...
  uint8_t to_api_resolution(VL53LMZResolution resolution) {
    if (resolution == VL53LMZ_4X4) {
      return VL53L5CX_RESOLUTION_4X4;
    } else if (resolution == VL53LMZ_8X8) {
      return VL53L5CX_RESOLUTION_8X8;
    } else {
  ...
  virtual uint8_t to_api_target_order(VL53LMZTargetOrder target_order) {
    if (target_order == VL53LMZ_STRONGEST) {
      return VL53L5CX_TARGET_ORDER_STRONGEST;
    } else if (target_order == VL53LMZ_CLOSEST) {
      return VL53L5CX_TARGET_ORDER_CLOSEST;
    } else {
      return VL53LMZ_INVALID_OPTION;
    }
  }

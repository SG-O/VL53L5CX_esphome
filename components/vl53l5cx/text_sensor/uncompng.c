...
#define UNCOMPNG__DATA_LEN__INCL_MAX 512u
...
uncompng__encode(int (*write_func)(void *context, const uint8_t *data_ptr, size_t data_len), void *context,
                 const uint8_t *pixel_ptr, size_t pixel_len, uint32_t width, uint32_t height, size_t stride,
...
static uint8_t uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX];

static void  //
uncompng__private_impl_initialize_buffer(uint32_t width, uint32_t height, uint32_t pixel_format) {
  ...
  uint32_t ihdr_crc32 = uncompng__private_impl_crc32_ieee(uncompng__private_impl_buffer + 0x000C, 0x001D - 0x000C);
  ...
  uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 4] = 0;
  uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 3] = 0;
  uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 2] = 0;
  uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 1] = 1;
}

static void  //
uncompng__private_impl_update_adler32(int ei, int ej) {
  uint32_t b = ((uint32_t) uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 4] << 8) |
               ((uint32_t) uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 3]);
  uint32_t a = ((uint32_t) uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 2] << 8) |
               ((uint32_t) uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 1]);
  while (ei < ej) {
    int end = ei + 5552;
    if (end > ej) {
      end = ej;
  ...
  uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 4] = (uint8_t) (b >> 8);
  uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 3] = (uint8_t) (b >> 0);
  uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 2] = (uint8_t) (a >> 8);
  uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 1] = (uint8_t) (a >> 0);
}

static int  //
uncompng__private_impl_flush(int (*write_func)(void *context, const uint8_t *data_ptr, size_t data_len), void *context,
                             int ej, bool final) {
  ...
  uncompng__private_impl_buffer[ei - 4] = 0x00u ^ (uint8_t) (deflate_block_len >> 0);
  uncompng__private_impl_buffer[ei - 3] = 0x00u ^ (uint8_t) (deflate_block_len >> 8);
  uncompng__private_impl_buffer[ei - 2] = 0xFFu ^ (uint8_t) (deflate_block_len >> 0);
  uncompng__private_impl_buffer[ei - 1] = 0xFFu ^ (uint8_t) (deflate_block_len >> 8);

  uncompng__private_impl_update_adler32(ei, ej);
  if (final) {
    uncompng__private_impl_buffer[ej + 0] = uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 4];
    uncompng__private_impl_buffer[ej + 1] = uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 3];
    uncompng__private_impl_buffer[ej + 2] = uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 2];
    uncompng__private_impl_buffer[ej + 3] = uncompng__private_impl_buffer[UNCOMPNG__DATA_LEN__INCL_MAX - 1];
    ej += 4;
  }

  uint32_t idat_crc32 =
      uncompng__private_impl_crc32_ieee(uncompng__private_impl_buffer + crc32_start, ej - crc32_start);
  ...
  if (!final) {
    int err0 = (*write_func)(context, uncompng__private_impl_buffer, (size_t) ej);
    if (err0 != 0) {
      return err0;
    }
...
static int  //
uncompng__private_impl_do_encode(int (*write_func)(void *context, const uint8_t *data_ptr, size_t data_len),
                                 void *context, const uint8_t *pixel_ptr, size_t pixel_len, uint32_t width,
                                 uint32_t height, size_t stride, uint32_t pixel_format) {
  static const int ei_first = 0x0030;
  static const int ei_later = 0x000D;
...
uncompng__encode(int (*write_func)(void *context, const uint8_t *data_ptr, size_t data_len), void *context,
  ...
  if (height > 0u) {
    ...
    uint64_t n = ((uint64_t) stride * (uint64_t) (height - 1u)) + (bytes_per_pixel * (uint64_t) width);
    if (pixel_len < n) {
      return UNCOMPNG__RESULT__INVALID_ARGUMENT;
    }
  }
  ...
  concurrent = true;
  int ret =
      uncompng__private_impl_do_encode(write_func, context, pixel_ptr, pixel_len, width, height, stride, pixel_format);
  concurrent = false;
  return ret;
}

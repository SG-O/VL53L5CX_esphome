...
static uint8_t _vl53l5cx_poll_for_answer(VL53L5CX_Configuration *p_dev, uint8_t size, uint8_t pos, uint16_t address,
                                         uint8_t mask, uint8_t expected_value) {
  uint8_t status = VL53L5CX_STATUS_OK;
  uint8_t timeout = 0;

  do {
    status |= VL53L5CX_RdMulti(&(p_dev->platform), address, p_dev->temp_buffer, size);
...
static uint8_t _vl53l5cx_send_offset_data(VL53L5CX_Configuration *p_dev, uint8_t resolution) {
  ...
  status |= VL53L5CX_WrMulti(&(p_dev->platform), 0x2e18, p_dev->temp_buffer, VL53L5CX_OFFSET_BUFFER_SIZE);
...
static uint8_t _vl53l5cx_send_xtalk_data(VL53L5CX_Configuration *p_dev, uint8_t resolution) {
  ...
  status |= VL53L5CX_WrMulti(&(p_dev->platform), 0x2cf8, p_dev->temp_buffer, VL53L5CX_XTALK_BUFFER_SIZE);
  status |= _vl53l5cx_poll_for_answer(p_dev, 4, 1, VL53L5CX_UI_CMD_STATUS, 0xff, 0x03);
...
uint8_t vl53l5cx_init(VL53L5CX_Configuration *p_dev) {
  ...
  status |=
      VL53L5CX_WrMulti(&(p_dev->platform), 0x2fd8, (uint8_t *) VL53L5CX_GET_NVM_CMD, sizeof(VL53L5CX_GET_NVM_CMD));
  status |= _vl53l5cx_poll_for_answer(p_dev, 4, 0, VL53L5CX_UI_CMD_STATUS, 0xff, 2);
  status |= VL53L5CX_RdMulti(&(p_dev->platform), VL53L5CX_UI_CMD_START, p_dev->temp_buffer, VL53L5CX_NVM_DATA_SIZE);
  ...
  status |= VL53L5CX_WrMulti(&(p_dev->platform), 0x2c34, p_dev->default_configuration,
                             sizeof(VL53L5CX_DEFAULT_CONFIGURATION));
  status |= _vl53l5cx_poll_for_answer(p_dev, 4, 1, VL53L5CX_UI_CMD_STATUS, 0xff, 0x03);

  status |=
      vl53l5cx_dci_write_data(p_dev, (uint8_t *) &pipe_ctrl, VL53L5CX_DCI_PIPE_CONTROL, (uint16_t) sizeof(pipe_ctrl));
#if VL53L5CX_NB_TARGET_PER_ZONE != 1
  tmp = VL53L5CX_NB_TARGET_PER_ZONE;
  status |=
      vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L5CX_DCI_FW_NB_TARGET, 16, (uint8_t *) &tmp, 1, 0x0C);
#endif

  status |= vl53l5cx_dci_write_data(p_dev, (uint8_t *) &single_range, VL53L5CX_DCI_SINGLE_RANGE,
...
uint8_t vl53l5cx_set_i2c_address(VL53L5CX_Configuration *p_dev, uint16_t i2c_address) {
  ...
  p_dev->platform.address_ = i2c_address;
  status |= VL53L5CX_WrByte(&(p_dev->platform), 0x7fff, 0x02);
...
uint8_t vl53l5cx_start_ranging(VL53L5CX_Configuration *p_dev) {
  ...
  uint32_t output[] = {VL53L5CX_START_BH,        VL53L5CX_METADATA_BH,       VL53L5CX_COMMONDATA_BH,
                       VL53L5CX_AMBIENT_RATE_BH, VL53L5CX_SPAD_COUNT_BH,     VL53L5CX_NB_TARGET_DETECTED_BH,
                       VL53L5CX_SIGNAL_RATE_BH,  VL53L5CX_RANGE_SIGMA_MM_BH, VL53L5CX_DISTANCE_BH,
                       VL53L5CX_REFLECTANCE_BH,  VL53L5CX_TARGET_STATUS_BH,  VL53L5CX_MOTION_DETECT_BH};
  ...
  status |= vl53l5cx_dci_write_data(p_dev, (uint8_t *) &(header_config), VL53L5CX_DCI_OUTPUT_CONFIG,
                                    (uint16_t) sizeof(header_config));

  status |= vl53l5cx_dci_write_data(p_dev, (uint8_t *) &(output_bh_enable), VL53L5CX_DCI_OUTPUT_ENABLES,
  ...
  status |=
      VL53L5CX_WrMulti(&(p_dev->platform), VL53L5CX_UI_CMD_END - (uint16_t) (4 - 1), (uint8_t *) cmd, sizeof(cmd));
  status |= _vl53l5cx_poll_for_answer(p_dev, 4, 1, VL53L5CX_UI_CMD_STATUS, 0xff, 0x03);
...
uint8_t vl53l5cx_stop_ranging(VL53L5CX_Configuration *p_dev) {
  ...
  status |= VL53L5CX_RdMulti(&(p_dev->platform), 0x2FFC, (uint8_t *) &auto_stop_flag, 4);
...
uint8_t vl53l5cx_get_ranging_data(VL53L5CX_Configuration *p_dev, VL53L5CX_ResultsData *p_results) {
  ...
  status |= VL53L5CX_RdMulti(&(p_dev->platform), 0x0, p_dev->temp_buffer, p_dev->data_read_size);
  p_dev->streamcount = p_dev->temp_buffer[0];
...
uint8_t vl53l5cx_set_ranging_frequency_hz(VL53L5CX_Configuration *p_dev, uint8_t frequency_hz) {
  ...
  status |=
      vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L5CX_DCI_FREQ_HZ, 4, (uint8_t *) &frequency_hz, 1, 0x01);
...
uint8_t vl53l5cx_set_integration_time_ms(VL53L5CX_Configuration *p_dev, uint32_t integration_time_ms) {
  ...
  if ((integration < (uint32_t) 2) || (integration > (uint32_t) 1000)) {
    ...
    status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L5CX_DCI_INT_TIME, 20, (uint8_t *) &integration,
                                        4, 0x00);
  }
...
uint8_t vl53l5cx_set_sharpener_percent(VL53L5CX_Configuration *p_dev, uint8_t sharpener_percent) {
  ...
  if (sharpener_percent >= (uint8_t) 100) {
    ...
    status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L5CX_DCI_SHARPENER, 16, (uint8_t *) &sharpener,
                                        1, 0xD);
  }
...
uint8_t vl53l5cx_set_target_order(VL53L5CX_Configuration *p_dev, uint8_t target_order) {
  ...
  if ((target_order == (uint8_t) VL53L5CX_TARGET_ORDER_CLOSEST) ||
    ...
    status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L5CX_DCI_TARGET_ORDER, 4,
...
uint8_t vl53l5cx_enable_internal_cp(VL53L5CX_Configuration *p_dev) {
  ...
  status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L5CX_DCI_INTERNAL_CP, 16,
...
uint8_t vl53l5cx_get_VHV_repeat_count(VL53L5CX_Configuration *p_dev, uint32_t *p_repeat_count) {
  uint8_t status = VL53L5CX_STATUS_OK;
  status |= vl53l5cx_dci_read_data(p_dev, (uint8_t *) p_dev->temp_buffer, VL53L5CX_DCI_VHV_CONFIG, 16);
...
uint8_t vl53l5cx_set_VHV_repeat_count(VL53L5CX_Configuration *p_dev, uint32_t repeat_count) {
  uint8_t status = VL53L5CX_STATUS_OK;
  status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L5CX_DCI_VHV_CONFIG, 16, (uint8_t *) &repeat_count,
                                      4, 0x4);
  return status;
}
...
uint8_t vl53l5cx_dci_read_data(VL53L5CX_Configuration *p_dev, uint8_t *data, uint32_t index, uint16_t data_size) {
  ...
  if ((data_size + (uint16_t) 12) > (uint16_t) VL53L5CX_TEMPORARY_BUFFER_SIZE) {
    ...
    status |= VL53L5CX_WrMulti(&(p_dev->platform), (VL53L5CX_UI_CMD_END - (uint16_t) 11), cmd, sizeof(cmd));
    status |= _vl53l5cx_poll_for_answer(p_dev, 4, 1, VL53L5CX_UI_CMD_STATUS, 0xff, 0x03);

    /* Read new data sent (4 bytes header + data_size + 8 bytes footer) */
    status |= VL53L5CX_RdMulti(&(p_dev->platform), VL53L5CX_UI_CMD_START, p_dev->temp_buffer, rd_size);
...
uint8_t vl53l5cx_dci_write_data(VL53L5CX_Configuration *p_dev, uint8_t *data, uint32_t index, uint16_t data_size) {
  ...
  if ((data_size + (uint16_t) 12) > (uint16_t) VL53L5CX_TEMPORARY_BUFFER_SIZE) {
    ...
    /* Send data to FW */
    status |= VL53L5CX_WrMulti(&(p_dev->platform), address, p_dev->temp_buffer,
...
uint8_t vl53l5cx_dci_replace_data(VL53L5CX_Configuration *p_dev, uint8_t *data, uint32_t index, uint16_t data_size,
